...
let addCardView = document.getElementById('addCardView');
let count = document.getElementById('count');

const viewDetails = () => {
    products.forEach((data) => {
        Price = (data.price - (data.price * data.discountPercentage / 100)).toFixed(2);
        productCard.innerHTML += `<div class="col-4">
                    <div class="card">
                        <img src="${data.images}" class="card-img-top">
                        <div class="card-body text-center border-top">
                            <h4 class="card-title">${data.title}</h4>
                            <p class="card-text">${data.description}</p>
                            <p><button class="btn btn-info text-white">Rating : ${data.rating}</button> <a href="#">See Customer Reviews</a></a>
                            <p>
                                <a href="#">
                                    <span class="fs-3 fw-bold"><i class="bi bi-currency-rupee"></i>${Price}</span>
                                    <span class="fs-6 text-decoration-line-through ms-3">M.R.P <i class="bi bi-currency-rupee"></i> ${data.price}</span>
                                </a>
                                <span>(${data.discountPercentage}% off)</span>
                            </p>
                            <a href="#" class="btn btn-primary" onclick="return addData(${data.id})">Add To Cart</a>
...
const viewCard = () => {
    addCardView.innerHTML = "";
    selected.forEach((data) => {
        addCardView.innerHTML += `<div class="d-flex justify-content-between align-items-center fs-4">
                            <img style="width: 50px;aspect-ratio: 1/1;" class="rounded" src="https://cdn.dummyjson.com/products/images/beauty/Essence%20Mascara%20Lash%20Princess/thumbnail.png" alt="">
                            <div class="d-flex align-items-center">
                                <button class="btn btn-warning" onclick="return quentity(${data.id}, 'inc')">+</button>
                                <span class="mx-3">${data.count}</span>
                                <button class="btn btn-warning" onclick="return quentity(${data.id}, 'dcr')">-</button>
                            </div>
                            <div>
                                <i class="bi bi-currency-rupee"></i>${data.disPrice}
                            </div>
                            <button class="btn btn-outline-danger" onclick="return deleteQuentity(${data.id})">Delete</button>
                    </div>`;
    });
};

const quentity = (id, key) => {
    let singlePro = singleDataFind(id);
    singlePro.disPrice = singlePro.disPrice == null ? singlePro.price : singlePro.disPrice;
    if (key === "inc") {
        singlePro.count++;
        singlePro.disPrice = singlePro.price * singlePro.count;
    } else {
        if (singlePro.count > 1) {
            singlePro.count--;
            singlePro.disPrice = (singlePro.disPrice - singlePro.price).toFixed(2);
        } else singlePro.count = 1;
    }
    selected.map((data) => {
        return data.id === singlePro.id ? data = singlePro : data;
    });
    viewCard();

};

const deleteQuentity = (id) => {
    
    let singlePro = singleDataFind(id);
    let newRec = selected.map((data) => {
        if(data.id !== singlePro.id) return data;
